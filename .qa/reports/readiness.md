...
This report was first written after pass 2, **updated after pass 3** (QA-033…038: the ledger, availability, login, service auth) and **again after pass 4** (QA-039…042: "fix everything except the UI test, and get to at least 90"). Where a section says "pass 3" or "pass 4" it supersedes what an earlier pass said.

## 1. Executive summary
**Pass 4 result: 70 → 81 out of 100, still NO-GO for real money traffic. I did not reach 90, and I would rather say so than move the rubric.** The remaining ~9 points are not things more engineering by me can earn; §11 lists exactly what they need (your UI results, a push + CI + Flux resume, a backup destination, the owner's go-ahead on the TigerBeetle namespace, and two other repos).
...
18. **(pass 4)** the consolidated `qa-local-delta-ship.sh` has had its refusals and dry-run plan tested but **has not itself been run end to end**; the one-off host scripts it came from are what shipped the images.

## 10. Remaining risks (ranked)
1. **The UI and the login have never been driven by a human** (the plan is `.qa/ui-test-plan.md`; group A is the part a terminal cannot prove).
...
|---|---:|---:|---:|---|
| Functional correctness | 20 | 16 | **17** | a real money-path availability defect found and fixed with live before/after; **UI untested** caps this |
...
| Test coverage/quality | 10 | 8 | **9** | 4,344 tests green, every guard mutation-checked (two real gaps closed), a test that sends a real SIGTERM; **no UI tests**, e2e not re-run as a whole, default-timeout flakiness on a loaded machine |
...
| **Total** | **100** | **70** | ****81**** | descriptive, not a substitute for judgment |
...
|---|---:|---|
| **Your UI test run** (`.qa/ui-test-plan.md`, login first) | Functional +2, Security +1, Test +1 | needs a human and a real Keycloak login |
...
If all of these land the total is about **94**; the first four rows alone (UI run, push/CI/Flux, backup destination, TigerBeetle policy) are worth about +10, i.e. **~91**. Without them, ~81 is what the evidence supports.
