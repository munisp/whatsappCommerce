...
This report was first written after pass 2, **updated after pass 3** (QA-033…038: the ledger, availability, login, service auth) and **again after pass 4** (QA-039…042: "fix everything except the UI test, and get to at least 90"), **once more for the UI fixes** (QA-043: the 403s, the missing sidebars, registration details), **again after pass 5** (QA-044…046: the TigerBeetle NetworkPolicy applied and verified live, a real TigerBeetle-side fault found and fixed along the way, the monitoring-alerts PR merged and confirmed live, the push completed and CI green, and a systematic "is every component genuinely used" pass that fixed Temporal and partially fixed Fluvio), and **once more after pass 6** ("do the remaining fixes apart from the UI test run": Fluvio is now genuinely live end to end, QA-047; the `tb-adapter` ImagePolicy gap is closed and the Flux-resume gate now reports SAFE; a Postgres-namespace policy was investigated and deliberately not applied; real ingress numbers were measured). Where a section says "pass N" it supersedes what an earlier pass said.

## 1. Executive summary
**Score: 70 → 82 → 85 → 85 out of 100, still NO-GO for real money traffic.** Pass 6 did real, verified engineering work — Fluvio now genuinely works end to end (two real cluster-deployment bugs found and fixed, not app-code bugs); the `tb-adapter` ImagePolicy gap is closed (the Flux-resume gate went from 3 blockers to **SAFE TO RESUME: yes**); a Postgres NetworkPolicy was scoped and deliberately not attempted this pass (see below) — **but the score stays at 85.** Neither Fluvio nor the ImagePolicy fix was ever one of the two things actually keeping this NO-GO for money traffic (your UI test run and a real off-host backup); moving the number for infrastructure hygiene that doesn't touch either of those would be exactly the rubric-moving this report has refused to do at every pass. Flux itself was **not** resumed — the gate being SAFE for the one blocker it tracks doesn't mean resuming is safe *right now*: 8 services are running hand-shipped images ahead of what's in git, and resuming would revert them, including this session's own fixes.
...
- **Pass 5 (QA-044…046), all live and verified on the cluster:**
  ...
  - **Pass 6 — Fluvio finished, for real (QA-047).** With the `fluvio` namespace confirmed to also be ours, the "zero SPUs" gap from pass 5 turned out to be two real, fixable cluster-deployment bugs: the SPU's storage class (`rook-ceph-block`, from the Helm chart's defaults) doesn't exist on this `kind` cluster — patched a ConfigMap, no restart needed, an SPU came up clean on the first real try; then the SPU's public endpoint turned out to have no external ingress configured, so the client needed `use_spu_local_address` (the SDK's switch for exactly this) to use the in-cluster address that was sitting right there, unused. Verified twice — a watchdog-timed diagnostic run inside the cluster, then a full real round trip (produce → land in Fluvio's own storage → consumer reads it back at offset 0 → forwards it) — and shipped. The live pod now shows all 5 topics connected within 50ms of boot, 0 restarts, 0 warnings since.
  - **Pass 6 — the `tb-adapter` ImagePolicy gap is closed.** CI already builds this image; nothing was watching the registry for it. Created the ImagePolicy/ImageRepository (matching how every other service's was set up — none of these are git-managed files, they're applied directly), updated the manifest off a real CI tag. `scripts/fluxResumePreflight.ts` went from **3 blockers** to **SAFE TO RESUME: yes**.
  - **Pass 6 — Flux was deliberately NOT resumed.** The gate being safe for the blocker it explicitly tracks isn't the same as "safe to resume right now": it still reports **8 warnings** — every hand-shipped `qa-local*` service (including this session's own Fluvio and TigerBeetle-adjacent fixes) is ahead of what's committed in git, and resuming would revert every one of them. That's a real decision, not a side effect of closing one gate.
  - **Pass 6 — a Postgres-namespace NetworkPolicy was scoped and deliberately not attempted.** The instance hosts dozens of tenants' databases (found via `pg_database`: `blueeco_*`, `ucard_*`, `lanai`, `gds`, `inec`, `tourismpay`, `vpp_e2e`, and more) — an order of magnitude more consumers to map correctly than TigerBeetle's 3. Postgres also has real credential-based authentication (TigerBeetle's protocol has none), so this is defense-in-depth, not an open door — lower urgency, and a wrong allowlist here risks a much wider outage. Also found: `pg-oracle-rw` is a **NodePort** service (32022) — externally reachable, same class of exposure as TigerBeetle's NodePort 32001 was.
  - **Pass 6 — real ingress numbers, for the SLO conversation.** Over the last hour of (mostly this session's own test) traffic: p50 48ms, p95 95ms, p99 99ms, 0% 5xx. No APISIX-native metrics are scraped (only app-level `http_requests_total`), so this measures just past the gateway, not the WAF/ingress hop itself. Also found in passing: several internal services (`ledger-bridge`, `payment-orchestrator`, `recon-worker`, others) have `/direct/<service>/*` ApisixRoutes reachable from the public internet, bypassing the main gateway — each should be protected by the fail-closed internal-key auth verified earlier in this engagement, but that specific claim (every one of them, not just ledger-bridge) has not been individually re-verified.

**Decision: NO-GO for production traffic that moves real money — the two real reasons are unchanged.** (1) **nothing has been driven through a real browser** (the login and every page); (2) **recoverability** — no off-host copy, no PITR, no TigerBeetle backup. Both narrowed but resolved: (3) ~~the shared TigerBeetle namespace has no network policy~~ **closed in pass 5**; (4) ~~image supply chain~~ **pushed, CI green, and the `tb-adapter` ImagePolicy gap is closed (pass 6)** — Flux itself stays suspended by deliberate choice, not because it's blocked. Fine to continue as dev/staging.
...
2. ~~The shared TigerBeetle namespace is open to any pod~~ **CLOSED in pass 5**: the NetworkPolicy is applied and verified live (89+ min clean, 100% write availability before/after). **The Postgres namespace (`postgres-oracle`) is still open to any pod — scoped in pass 6, not yet applied**: it hosts dozens of tenants' databases (an order of magnitude more consumers than TigerBeetle's 3 to map correctly), and unlike TigerBeetle it has real credential auth, so this is defense-in-depth, not an open door. `pg-oracle-rw` is also a **NodePort** (32022) — externally reachable, same exposure class as TigerBeetle's old NodePort 32001. TigerBeetle's own wire protocol still has no authentication of its own; the NetworkPolicy is what's standing in for it.
3. **Recoverability:** no off-host backup destination, no PITR, no TigerBeetle backup. Unchanged — waiting on a bucket + credentials (in-cluster storage was considered and correctly rejected: it wouldn't survive losing the cluster/host, which is the actual risk this addresses).
4. **Image supply chain:** commits are pushed (0 unpushed), CI is green, and **the `tb-adapter` ImagePolicy gap is closed (pass 6)** — `scripts/fluxResumePreflight.ts` now reports **SAFE TO RESUME: yes (no blockers)**, down from 3. **Flux was deliberately not resumed**: the gate still reports 8 warnings — every hand-shipped service (server, ledger-bridge, tb-adapter, recon-worker, payment-orchestrator, commerce-engine, event-processor, fluvio-consumer) is running an image ahead of what's committed in git, and resuming would revert all of them, including this session's own fixes. Resuming needs a deliberate decision (and likely a round of "update git to match what's actually running" first), not a side effect of closing one gate.
5. ~~Fluvio has zero SPUs registered~~ **CLOSED in pass 6 (QA-047)**: two real cluster-deployment bugs (a storage class that doesn't exist on this cluster; an SPU with no external ingress and a client not told to use the in-cluster address instead), both fixed, verified with a real end-to-end message round trip, shipped live.
6. **No admission control for overload:** one process has none. The ingress itself was measured this pass (p50 48ms / p95 95ms / p99 99ms / 0% 5xx over the last hour) but that traffic is mostly this session's own testing, not organic load — treat as a starting point for SLO discussion, not a load-tested number. Several internal services have public `/direct/<service>/*` routes bypassing the main gateway (found in passing) — each should be covered by the fail-closed internal-key auth verified earlier, but that has only been individually re-confirmed for `ledger-bridge`.
...
9. **Unfixed findings:** stranded `crp:` marker liveness; `commerce-engine` unwired; `paymentTransactions` has no ledger link (an owner decision); `payment-orchestrator`'s stale bridge contract; `lanai`/`vpp` list a single TigerBeetle address; Redis has no per-app keyspace isolation (shares db0 with `lanai`/`alfred`/`velma`); the Postgres namespace's NodePort and open-to-any-pod exposure (both found, not yet fixed).
10. Unexercised: UI/accessibility/browser compatibility, network faults, a live load test at real (not self-generated) traffic, a soak beyond 10 minutes (local), the Postgres NetworkPolicy, per-route re-verification of the `/direct/*` exposures.
...
**What it would take to reach 90 — and why I could not get there alone.** Each item is something only you (or another namespace's owner) can do or authorise, or a decision I should not make unilaterally. Five rows from earlier versions of this table are gone because they're done:
...
| **A backup destination** (a bucket + credentials) and a **TigerBeetle backup** you have restored once | DR +1, Data integrity +1 | the code is written; it needs credentials I must not invent — in-cluster storage was offered and correctly declined (doesn't survive losing the cluster) |
| **A decision to resume Flux** (after reconciling the 8 warnings — hand-shipped tags vs. what's in git) | Reliability +1 | outward-facing, affects every service at once; the gate says the one blocker it tracks is clear, not that resuming right now is safe |
| **The Postgres-namespace policy** — same shape as the TigerBeetle one, scoped this pass, needs its own careful runbook (many more consumers) and your go-ahead before applying | Security +1 | shared infra, dozens of tenants; a mistake here is a much wider outage than TigerBeetle's was |
| **SLOs you sign off** (real p50/p95/p99/error-rate numbers are now measured, §10 item 6) + a real (not self-generated) load test | Performance +1, Scalability +1 | thresholds are stakeholder decisions |
...
| ~~An ImagePolicy for `tb-adapter`~~ | — | **done, pass 6** |
| ~~Someone who owns the `fluvio` namespace registers a working SPU~~ | — | **done, pass 6 (QA-047) — it was ours after all** |

If all of the remaining rows land the total is about **91**; the UI run and the backup destination alone are worth about +5, i.e. **~90**. Without them, **85** is what the evidence supports.
...
BLOCKERS (each flips the answer if closed):
  ...
  3. A deliberate decision to resume Flux, after reconciling 8 services' hand-shipped tags against what's in git (the
     specific gate this row used to track — the tb-adapter ImagePolicy — is closed as of pass 6; resuming itself was not
     done, on purpose: `scripts/fluxResumePreflight.ts` reports SAFE for its one tracked blocker but still lists 8
     warnings that resuming would act on).
  4. Postgres namespace (`postgres-oracle`) is still open to any pod — same category of exposure TigerBeetle had, scoped
     in pass 6 (dozens of tenants, real auth unlike TigerBeetle) but not yet applied.
...
CLOSED IN PASS 6: the Fluvio cluster's missing SPU (two real deploy bugs, fixed, verified end to end, QA-047); the
     tb-adapter ImagePolicy gap (Flux-resume gate: 3 blockers → 0).
HIGH-RISK ISSUES: the open Postgres namespace (+ its NodePort); the not-yet-resumed Flux (deliberately, pending the
     8-warning reconciliation); locally-shipped images still what's running everywhere; recoverability; unbounded
     overload at the ingress (now measured: p50 48ms/p95 95ms/p99 99ms/0% 5xx, but under self-generated test traffic).
