...
- **QA-012 / QA-013**: done — fixed, deployed, and confirmed live across all 14 services, including the `ledger-bridge` `/health` follow-up (it always returned HTTP 200 regardless of TigerBeetle/Postgres connectivity; now split into a shallow `/health` for liveness and a real `/health/ready` for readiness, matching `server`'s own pattern — see `.qa/defects.md` QA-013 for the full writeup, including the `server`-side blind spot this also closed). Worth a separate look sometime: why two manifest sets (`k8s/` and `k8s-flux/`) exist, since the unused one is now confirmed stale in more than one way.
