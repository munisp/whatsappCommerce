...
> **CORRECTION (2026-09-21, QA-040) — read before quoting any tail latency below.** The rows measured with a load generator on the laptop (every row in this file) reached the container through Docker Desktop's port proxy, and their worst-case latencies equal the test duration: a few connections were starved for the whole run. That is the client/proxy path, not the server. Re-measured with the driver **inside the docker network** (`.qa/perf/loadshed-compare.sh`, ABBA order, nothing else running): closed-loop `auth.me` at **c=800 gives p99 ≈ 0.85–0.95 s and max ≈ 1.0–1.15 s** at ~1,250–1,330 rps, so **the "no load shedding — p99 13.3 s" weak spot below was mostly a measurement artifact.** The throughput/saturation figures (~1.1–1.3k rps) still hold. What *is* real: under **open-loop** overload (2× capacity, thousands of connections) the queue grows without bound (served p50 7–10 s, p99 ≈ 56 s) — and an event-loop load shedder that I built did not change that, because the queue sits in the kernel accept/read path ahead of any middleware; it was removed. Raw runs: `.qa/perf/loadshed/`. Full account: `.qa/defects.md` QA-040.
