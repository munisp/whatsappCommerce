 ...
 * the full 38-journey suite alongside the unit tests.
 *
...
describe("WhatsApp feature simulation (38 journeys)", () => {
  it(
    "runs every journey against the real webhook handlers with Meta mocked",
    async () => {
      const journeys = await loadJourneys();
      expect(journeys.length).toBe(38);
    ...
    20 * 60 * 1000, // 20 min — boots PGlite + server once, then all 38
  );
});
